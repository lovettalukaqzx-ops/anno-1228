...
typedef struct binary_fuse64_s {
  uint64_t Seed;
  uint32_t Size;
  uint32_t SegmentLength;
  uint32_t SegmentLengthMask;
...
static inline uint64_t binary_fuse64_fingerprint(uint64_t hash) {
  return hash;
}

static inline binary_hashes_t binary_fuse64_hash_batch(uint64_t hash,
                                        const binary_fuse64_t *filter) {
  uint64_t hi = binary_fuse_mulhi(hash, filter->SegmentCountLength);
  binary_hashes_t ans;
  ans.h0 = (uint32_t)hi;
  ans.h1 = ans.h0 + filter->SegmentLength;
...
static inline uint32_t binary_fuse64_hash(uint64_t index, uint64_t hash,
                                        const binary_fuse64_t *filter) {
    uint64_t h = binary_fuse_mulhi(hash, filter->SegmentCountLength);
    h += index * filter->SegmentLength;
    // keep the lower 36 bits
...
// Report if the key is in the set, with false positive rate.
static inline bool binary_fuse64_contain(uint64_t key,
                                        const binary_fuse64_t *filter) {
  uint64_t hash = binary_fuse_mix_split(key, filter->Seed);
  uint64_t f = binary_fuse64_fingerprint(hash);
  binary_hashes_t hashes = binary_fuse64_hash_batch(hash, filter);
  f ^= filter->Fingerprints[hashes.h0] ^
       filter->Fingerprints[hashes.h1] ^
       filter->Fingerprints[hashes.h2];
  return f == 0;
}
...
// allocate enough capacity for a set containing up to 'size' elements
// caller is responsible to call binary_fuse64_free(filter)
// size should be at least 2.
static inline bool binary_fuse64_allocate(uint32_t size,
                                         binary_fuse64_t *filter) {
  uint32_t arity = 3;
  filter->Size = size;
  filter->SegmentLength = size == 0 ? 4 : binary_fuse_calculate_segment_length(arity, size);
  if (filter->SegmentLength > 262144) {
  ...
  filter->SegmentCountLength = filter->SegmentCount * filter->SegmentLength;
  filter->Fingerprints =
      (uint64_t *)calloc(filter->ArrayLength, sizeof(uint64_t));
  return filter->Fingerprints != NULL;
}

// report memory usage
static inline size_t binary_fuse64_size_in_bytes(const binary_fuse64_t *filter) {
  return filter->ArrayLength * sizeof(uint64_t) + sizeof(binary_fuse64_t);
}

// release memory
static inline void binary_fuse64_free(binary_fuse64_t *filter) {
  free(filter->Fingerprints);
  filter->Fingerprints = NULL;
  filter->Seed = 0;
  filter->Size = 0;
...
// Construct the filter, returns true on success, false on failure.
// The algorithm fails when there is insufficient memory.
// The caller is responsable for calling binary_fuse64_allocate(size,filter)
// before. For best performance, the caller should ensure that there are not too
// many duplicated keys.
static inline bool binary_fuse64_populate(uint64_t *keys, uint32_t size,
                           binary_fuse64_t *filter) {
  if (size != filter->Size) {
    return false;
  }
  ...
  for (int loop = 0; true; ++loop) {
    ...
    for (uint32_t i = 0; i < size; i++) {
      uint64_t hash = reverseOrder[i];
      uint32_t h0 = binary_fuse64_hash(0, hash, filter);
      t2count[h0] += 4;
      t2hash[h0] ^= hash;
      uint32_t h1= binary_fuse64_hash(1, hash, filter);
      t2count[h1] += 4;
      t2count[h1] ^= 1U;
      t2hash[h1] ^= hash;
      uint32_t h2 = binary_fuse64_hash(2, hash, filter);
      t2count[h2] += 4;
      t2hash[h2] ^= hash;
      t2count[h2] ^= 2U;
      if ((t2hash[h0] & t2hash[h1] & t2hash[h2]) == 0) {
    ...
    while (Qsize > 0) {
      ...
      if ((t2count[index] >> 2U) == 1) {
        ...
        //h012[0] = binary_fuse64_hash(0, hash, filter);
        h012[1] = binary_fuse64_hash(1, hash, filter);
        h012[2] = binary_fuse64_hash(2, hash, filter);
        h012[3] = binary_fuse64_hash(0, hash, filter); // == h012[0];
        h012[4] = h012[1];
        uint8_t found = t2count[index] & 3U;
        reverseH[stacksize] = found;
        reverseOrder[stacksize] = hash;
  ...
  for (uint32_t i = size - 1; i < size; i--) {
    // the hash of the key we insert next
    uint64_t hash = reverseOrder[i];
    uint64_t xor2 = binary_fuse64_fingerprint(hash);
    uint8_t found = reverseH[i];
    h012[0] = binary_fuse64_hash(0, hash, filter);
    h012[1] = binary_fuse64_hash(1, hash, filter);
    h012[2] = binary_fuse64_hash(2, hash, filter);
...
static inline size_t binary_fuse64_serialization_bytes(const binary_fuse64_t *filter) {
  return sizeof(filter->Seed) + sizeof(filter->Size) + sizeof(filter->SegmentLength) +
   sizeof(filter->SegmentCount) +
        sizeof(filter->SegmentCountLength) + sizeof(filter->ArrayLength) +
        sizeof(uint64_t) * filter->ArrayLength;
}

// serialize a filter to a buffer, the buffer should have a capacity of at least
// binary_fuse64_serialization_bytes(filter) bytes.
// Native endianess only.
static inline void binary_fuse64_serialize(const binary_fuse64_t *filter, char *buffer) {
  memcpy(buffer, &filter->Seed, sizeof(filter->Seed));
  buffer += sizeof(filter->Seed);
  memcpy(buffer, &filter->Size, sizeof(filter->Size));
  buffer += sizeof(filter->Size);
  ...
  memcpy(buffer, &filter->ArrayLength, sizeof(filter->ArrayLength));
  buffer += sizeof(filter->ArrayLength);
  memcpy(buffer, filter->Fingerprints, filter->ArrayLength * sizeof(uint64_t));
}

// deserialize the main struct fields of a filter from a buffer, returns the buffer position
// immediately after those fields. If you used binary_fuse64_serialize the return value will point at
// the start of the `Fingerprints` array. Use this option if you want to allocate your own memory or
// perhaps have the memory `mmap`ed to a file. Nothing is allocated. Do not call binary_fuse64_free
// on the returned pointer. Native endianess only.
static inline const char* binary_fuse64_deserialize_header(binary_fuse64_t* filter, const char* buffer) {
  memcpy(&filter->Seed, buffer, sizeof(filter->Seed));
  buffer += sizeof(filter->Seed);
  memcpy(&filter->Size, buffer, sizeof(filter->Size));
  buffer += sizeof(filter->Size);
...
// deserialize a filter from a buffer, returns true on success, false on failure.
// The output will be reallocated, so the caller should call binary_fuse64_free(filter) before
// if the filter was already allocated. The caller needs to call binary_fuse64_free(filter) after.
// The number of bytes read is binary_fuse64_serialization_bytes(output).
// Native endianess only.
static inline bool binary_fuse64_deserialize(binary_fuse64_t * filter, const char *buffer) {
  const char* fingerprints = binary_fuse64_deserialize_header(filter, buffer);
  filter->Fingerprints = (uint64_t*)malloc(filter->ArrayLength * sizeof(uint64_t));
  if(filter->Fingerprints == NULL) {
    return false;
  }
  memcpy(filter->Fingerprints, fingerprints, filter->ArrayLength * sizeof(uint64_t));
  return true;
}
