...
template <typename T>
concept filter_type = std::same_as<T, binary_fuse8_t> || std::same_as<T, binary_fuse16_t> || std::same_as<T, binary_fuse64_t>;
...
struct ftype<binary_fuse8_t> {
  ...
  static constexpr auto* hash_batch          = binary_fuse8_hash_batch;
...
struct ftype<binary_fuse16_t> {
  ...
  static constexpr auto* hash_batch          = binary_fuse16_hash_batch;
...
template <>
struct ftype<binary_fuse64_t> {
  static constexpr auto* allocate            = binary_fuse64_allocate;
  static constexpr auto* populate            = binary_fuse64_populate;
  static constexpr auto* contains            = binary_fuse64_contain;
  static constexpr auto* hash_batch          = binary_fuse64_hash_batch;
  static constexpr auto* free                = binary_fuse64_free;
  static constexpr auto* serialization_bytes = binary_fuse64_serialization_bytes;
  static constexpr auto* serialize           = binary_fuse64_serialize;
  static constexpr auto* deserialize_header  = binary_fuse64_deserialize_header;
  using fingerprint_t                        = std::uint64_t;
};
...
class filter {
public:
  using fingerprint_t = typename ftype<FilterType>::fingerprint_t;
  ...
  void populate(std::span<const std::uint64_t> keys, std::uint64_t seed) {
    if (is_populated()) {
      throw std::runtime_error("filter is already populated. You must provide all data at once.");
    }
  ...
  [[nodiscard]] const fingerprint_t* data() const {
    return fil_.Fingerprints;
  }
  ...
  [[nodiscard]] std::size_t array_length() const {
    return static_cast<std::size_t>(fil_.ArrayLength);
  }

  [[nodiscard]] std::uint32_t segment_length() const { return fil_.SegmentLength; }
  [[nodiscard]] std::uint32_t segment_length_mask() const { return fil_.SegmentLengthMask; }
  [[nodiscard]] std::uint32_t segment_count_length() const { return fil_.SegmentCountLength; }
  ...
  [[nodiscard]] std::array<std::size_t, 3> positions(std::uint64_t key) const {
    ...
    const auto hash   = binary_fuse_mix_split(key, fil_.Seed);
    const auto hashes = ftype<FilterType>::hash_batch(hash, &fil_);
...
using filter64 = filter<binary_fuse64_t>;

using filter64_sink   = persistent_filter<binary_fuse64_t, mio::access_mode::write>;
using filter64_source = persistent_filter<binary_fuse64_t, mio::access_mode::read>;
