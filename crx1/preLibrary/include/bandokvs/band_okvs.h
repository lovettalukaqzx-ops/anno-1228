...
class BandOkvs {
 public:
  ...
  template<typename V>
  void Decode(const oc::block* keys, const V* okvs, V* out, int decodeSize) {
    if (band_length_ <= 128) {
      return DecodeHelper<uint<1>, V>(keys, okvs, out, decodeSize);
    } else if (band_length_ <= 256) {
      return DecodeHelper<uint<2>, V>(keys, okvs, out, decodeSize);
    } else if (band_length_ <= 384) {
      return DecodeHelper<uint<3>, V>(keys, okvs, out, decodeSize);
    } else if (band_length_ <= 512) {
      return DecodeHelper<uint<4>, V>(keys, okvs, out, decodeSize);
    } else if (band_length_ <= 640) {
      return DecodeHelper<uint<5>, V>(keys, okvs, out, decodeSize);
    } else if (band_length_ <= 768) {
      return DecodeHelper<uint<6>, V>(keys, okvs, out, decodeSize);
    }
 ...
 private:
  ...
  template<typename T, typename V>
  inline void DecodeHelper(const Band<T>* bands, const V* okvs, V* out, int decodeSize) {
    int n = decodeSize;
    for (int i = 0; i < n; ++i) {
  ...
  template<typename T, typename V>
  void DecodeHelper(const oc::block* keys, const V* okvs, V* out, int decodeSize) {
    int n = decodeSize;
